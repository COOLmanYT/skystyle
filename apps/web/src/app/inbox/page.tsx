  ...
  return (
    <main className="min-h-screen px-4 py-8" style={{ background: "var(--background)", color: "var(--foreground)" }}>
      <PageSpacingWrapper page="inbox" className="max-w-3xl mx-auto space-y-5">
        <div className="flex items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">🔔 Inbox</h1><p className="text-sm opacity-60">Recommendations, support updates, and system notices.</p></div><Link href="/dashboard" className="text-sm rounded-xl px-3 py-2" style={{ border: "1px solid var(--card-border)" }}>Back</Link></div>
        ...
        {loading ? <p className="text-sm opacity-50">Loading inbox…</p> : messages.length === 0 ? <p className="text-sm opacity-50">Your inbox is clear.</p> : <div className="space-y-3">{messages.map((message) => <article key={message.id} className="rounded-2xl p-4 space-y-2" style={{ background: "var(--card)", border: "1px solid var(--card-border)", opacity: message.read_at ? 0.7 : 1 }}><div className="flex justify-between gap-2"><h2 className="text-sm font-semibold">{message.title}</h2><time className="text-xs opacity-50">{new Date(message.created_at).toLocaleString()}</time></div><p className="text-sm whitespace-pre-wrap">{message.body}</p><div className="flex gap-3 text-xs"><button onClick={() => updateMessage(message.id, { read: true })} className="underline">Mark read</button><button onClick={() => updateMessage(message.id, { dismissed: true })} className="underline">Dismiss</button></div></article>)}</div>}
      </PageSpacingWrapper>
    </main>
  );
}
