...
type User = {
  ...
  usage: { ai_uses: number } | null; apiKeys: unknown[]; lastLoggedIn: string | null; lastApiUse: string | null; lastAiUse: string | null;
...
export default function DevUsers() {
  ...
  async function adjustCredits(user: User) {
    const type = window.prompt("Credit type: app, money, or api", "app");
    if (!type || !["app", "money", "api"].includes(type)) return;
    const amount = Number(window.prompt(type === "money" ? "Amount in cents to gift (negative to revoke)" : "Amount to gift (use a negative number to revoke)", "0"));
    if (!Number.isInteger(amount) || amount === 0) return;
    const apiKeyId = type === "api" ? window.prompt("API key ID to adjust (from the user profile)") : null;
    if (type === "api" && !apiKeyId) return;
    await updateUser(user, type === "app" ? { appCreditDelta: amount } : type === "money" ? { moneyCreditDelta: amount } : { apiCreditDelta: amount, apiKeyId });
  }
  async function restrictAccess(user: User, target: "app" | "api") {
    const until = window.prompt(`Block ${target} access until ISO date/time (leave blank for an indefinite block).`, "");
    if (until === null) return;
    await updateUser(user, until.trim() ? target === "app" ? { appBlockedUntil: until.trim() } : { apiBlockedUntil: until.trim() } : target === "app" ? { appBlocked: true } : { apiBlocked: true });
  }
  async function banUser(user: User) {
    const reason = window.prompt(`Permanently ban ${user.email || user.id}? They will be blocked from app and API access. Enter a reason.`);
    if (reason === null) return;
    await updateUser(user, { banned: true, banReason: reason, appBlocked: true, apiBlocked: true });
  }
  ...
  return <section className="space-y-5">
    ...
    {loading ? <p style={{ color: "var(--foreground)", opacity: .55 }}>Loading users…</p> : visibleUsers.map((user) => <article className="rounded-xl p-4 space-y-3" key={user.id} style={{ background: "var(--card)", border: "1px solid var(--card-border)", color: "var(--foreground)" }}><div className="flex flex-wrap items-start justify-between gap-3"><div><Link href={`/dev/operations/users/${user.id}`} className="text-sm font-semibold underline">{user.name || user.email || user.id}</Link>{user.name && user.email && <p className="text-xs opacity-60">{user.email}</p>}<p className="mt-1 text-xs opacity-60">Plan: {user.plan} · Joined: {user.joinedAt ? new Date(user.joinedAt).toLocaleDateString() : "Not recorded"} · AI total: {user.totalAiUses} · Today: {user.usage?.ai_uses ?? 0} · Feedback: {user.feedbackCount} · App credits: {user.credits} · API keys: {user.apiKeys.length}</p></div><div className="flex gap-2"><span className="rounded-full px-2 py-1 text-xs" style={{ background: user.pendingDeletion ? "#ff950033" : "var(--background)" }}>{user.pendingDeletion ? "Pending deletion" : "Active"}</span>{user.controls?.app_blocked && <span className="rounded-full px-2 py-1 text-xs" style={{ background: "#ff453a33" }}>App blocked</span>}{user.controls?.api_blocked && <span className="rounded-full px-2 py-1 text-xs" style={{ background: "#ff453a33" }}>API blocked</span>}</div></div><div className="flex flex-wrap gap-2"><select aria-label="User plan" value={user.plan === "dev" ? "dev" : user.plan} disabled={user.plan === "dev"} onChange={(event) => void updateUser(user, { plan: event.target.value })} className="rounded-lg px-2 py-1 text-xs" style={{ background: "var(--background)", border: "1px solid var(--card-border)" }}><option value="free">Free</option><option value="pro">Pro</option><option value="dev">Dev</option></select><button onClick={() => void restrictAccess(user, "app")} className="rounded-lg px-2 py-1 text-xs" style={{ border: "1px solid var(--card-border)" }}>Block App…</button><button onClick={() => void restrictAccess(user, "api")} className="rounded-lg px-2 py-1 text-xs" style={{ border: "1px solid var(--card-border)" }}>Block API…</button><button onClick={() => void adjustCredits(user)} className="rounded-lg px-2 py-1 text-xs" style={{ border: "1px solid var(--card-border)" }}>Adjust credits…</button><button onClick={() => void exportUser(user)} className="rounded-lg px-2 py-1 text-xs" style={{ border: "1px solid var(--card-border)" }}>Download data</button>{user.plan !== "dev" && <><button onClick={() => void banUser(user)} className="rounded-lg px-2 py-1 text-xs" style={{ border: "1px solid #ff9500", color: "#ff9500" }}>Ban</button><button onClick={() => void deleteUser(user)} className="rounded-lg px-2 py-1 text-xs" style={{ border: "1px solid #ff453a", color: "#ff453a" }}>Delete account</button></>}<label className="text-xs">App AI/day <input aria-label="App daily AI limit" type="number" min="0" defaultValue={user.controls?.app_daily_ai_limit ?? ""} onBlur={(event) => event.currentTarget.value !== "" && void updateUser(user, { appDailyAiLimit: Number(event.currentTarget.value) })} className="ml-1 w-14 rounded p-1" style={{ background: "var(--background)", border: "1px solid var(--card-border)" }} /></label><label className="text-xs">API/min <input aria-label="API per-minute limit" type="number" min="0" defaultValue={user.controls?.api_rate_limit_per_min ?? ""} onBlur={(event) => event.currentTarget.value !== "" && void updateUser(user, { apiRateLimitPerMin: Number(event.currentTarget.value) })} className="ml-1 w-14 rounded p-1" style={{ background: "var(--background)", border: "1px solid var(--card-border)" }} /></label></div></article>)}
